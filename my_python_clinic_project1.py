...
def parse_input(user_input):
    ...
    Returns:
    - str: The response generated by the appropriate function based on the detected pattern, or the default response if no pattern is matched.
    """
    ...
    personal_statement_pattern = re.compile(r"\b(i think\b|\bin my idea\b|\bmy opinion is\b|\bi believe\b|\bin my view\b|\bmy experience with\b)", re.IGNORECASE)
    dismissal_pattern = re.compile(r"\b(i don('*)t believe\b|\bi don('*)t agree\b|\not convinced\b|\bdisagreed\b|\bi find it hard to accept\b)\b", re.IGNORECASE)
    ...
    emotions_pattern = re.compile(r"\b(i feel|feeling|annoying|infuriating|irritating|frustrating|amazing|exciting|joyful|happy|positive|uplifting|sad|angry|confused)\b", re.IGNORECASE)
    ...
    you_are_pattern = re.compile(r"you are\s+([A-Za-z]+)|you('*)re\s+([A-Za-z]+)|you are not\s+([A-Za-z]+)|you('*)re not\s+([A-Za-z]+)", re.IGNORECASE)
    ...
    future_implications_pattern = re.compile(r"\b(in the future\b|\bwill lead to\b|\bconsequences will be\b)\b", re.IGNORECASE)
    seeking_advice_pattern = re.compile(r"\b(what should i do\b|\bany suggestions\b|\bwhat do you think\b)\b", re.IGNORECASE)
    ...
    if re.search(repetitions_pattern, user_input):
        ...
        return handle_personal_statment(user_input)
...
def handle_personal_statment(user_input):
    """
...
def handle_never_always(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a 'never' or 'always' pattern is detected, otherwise None
    """
...
def handle_emotions(user_input):
    ...
    """
    annoyings = ['infuriating', 'irritating', 'frustrating', 'angry', 'annoying']
    ...
    elif any(annoying in user_input for annoying in annoyings):
...
def handle_absolute_keyword(user_input):
    """
    Handles responses when the user's input includes absolute expressions like 'impossible'|'everyone knows'|'absolutely'.
    Args:
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if an absolute keyword pattern is detected, otherwise None
    """
...
def handle_futility(user_input):
    """
    Handles responses when the user's input includes expressions related to futility, such as 'pointless'|'silly'|'sense'(=refers to 'make no sense').
    Args:
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a futility keyword pattern is detected, otherwise None
    """
...
def handle_dont_understand(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a 'don't understand' pattern is detected, otherwise None
    """
    responses = [
...
def handle_right_wrong(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a 'right' or 'wrong' pattern is detected, otherwise None
    """
...
def handle_agreement_doubt_pattern(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if an agreement with doubt pattern is detected, otherwise None
    """
    responses = [
...
def handle_unexplored_areas_pattern(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if an unexplored areas pattern is detected, otherwise None
    """
    responses = [
...
def handle_future_implications_pattern(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a future implications pattern is detected, otherwise None
    """
    responses = [
...
def handle_seeking_advice_pattern(user_input):
    ...
    - user_input (str): The user's input
    Returns:
    - str or None: A randomly chosen response from a predefined set if a seeking advice pattern is detected, otherwise None
    """
    responses = [
...
def main():
    ...
    if start_argument.strip().lower() == "yes":
        argue_time = int(st.text_input("How many minutes would you like to argue? (enter the number of minutes): "))
