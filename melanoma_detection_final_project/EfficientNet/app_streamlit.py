...
def main():
    ...
    **How to Use:**
    ...
    :red[**(For precise result, it is important to edit the image before uploading it so that it focuses on the mole itself.)**]
    2. **Upload the Photo:** Use the upload feature to submit the captured image to the app.
    3. **Receive Prediction:** The model will analyze the uploaded image and estimate the likelihood of the mole being malignant or benign.
    """)
    ...
    st.write("""💡 **Please keep in mind that while the model is highly accurate (up to ~95% precision),
